...
it; and `retrieve` **matches** on the result rather than `?`-ing it — which is
also what lets the counter see the failure.

### The silent-total-failure mode to watch for on the first real call

Two MTN facts are unverified because the real sandbox has never been called:
the case of the `accountHolderIdType` path segment, and whether MTN answers
`404` for an unknown holder at all (it documents `200`, `401` and `500` only).
...
migration behind it**. Do not add one casually: this is the same fact rule 4
points at from the other side — a merchant enumerating the number space leaves
no record in vpay.

**3. Logs carry a masked number and never a name.** One line per request, at
`info` (`warn` on a rail failure), carrying the rail's code, `+2376••••200`
and whether a holder was found. **The bullet count is fixed, not one per hidden
...
need is in hand the day it is decided.
...
`Capabilities::supports_account_holder_lookup` is what the handler branches on
(ADR-0002) — there is no `if code == "mtn_momo"` arm. `mtn_momo` declares
`true`; `orange_money` declares `false` and inherits
`ProviderError::Unsupported`, **not** a `NotImplemented` token, because nothing
about Orange is unbuilt work someone owes: the flag is a permanent answer, and
Orange's equivalent route is merely unconfirmed from this repository. A rail
