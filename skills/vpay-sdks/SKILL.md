...
| `sdks/flutter/vpay_checkout_flutter` | `vpay_checkout_flutter`         | The mobile **payer** surface. Since 2026-09-17 ([vpay#192](https://github.com/vaam-apps/vpay/pull/192)) it renders the checkout as a **Flutter widget tree**, not the hosted page in a window — this row said the latter until then. The browser is now only the redirect handler for a rail like Orange. Material 3 since [vpay#198](https://github.com/vaam-apps/vpay/pull/198), themed through `VpayCheckoutTheme`. Still reports a typed result once the intent actually settles | **`publish_to: none`** (2026-09-13)                                                                                                                  |
